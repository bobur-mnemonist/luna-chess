...
class ChessBoard(GridLayout):
    def __init__(self, luna_panel, difficulty='medium', on_game_over=None, **kwargs):
        ...
        self.build_board()
        self.refresh_board()
        self.luna_panel.set_mood(random.choice(['love', 'smile', 'cheer']), duration=2)
    ...
    def on_square_press(self, instance):
        ...
        if move in self.board.legal_moves:
            ...
            if piece and piece.color == chess.WHITE:
                ...
                self.selected_square = None
                self.luna_panel.set_mood('angry', duration=1.5)
    ...
    def make_user_move(self, move):
        ...
        if self.move_count <= 2 and uci in LUNA_OPENING_COMMENTS:
            ...
            self.luna_panel.set_mood(random.choice(['excited', 'cheer']), duration=2)
            self.say(random.choice(LUNA_USER_CAPTURE))
        else:
            self.luna_panel.set_mood('smile', duration=1.5)
    ...
    def _apply_ai_move(self, ai_move):
        ...
        if self.board.is_check():
            self.luna_panel.set_mood('wink', duration=2)
            self.say(random.choice(LUNA_CHECK))
        elif ai_capture:
            self.luna_panel.set_mood(random.choice(['smile', 'wink']), duration=2)
