...
class ChessBoard(GridLayout):
    def __init__(self, luna_panel, difficulty='medium', on_game_over=None, **kwargs):
        ...
        self.build_board()
        self.refresh_board()
        self.luna_panel.set_mood(random.choice(['love', 'smile', 'cheer']))
    ...
    def _handle_square_press(self, instance):
        ...
        if move in self.board.legal_moves:
            ...
            if piece and piece.color == chess.WHITE:
                ...
                self.selected_square = None
                self.luna_panel.set_mood('angry')
    ...
    def make_user_move(self, move):
        ...
        if self.move_count <= 2 and uci in LUNA_OPENING_COMMENTS:
            ...
            self.luna_panel.set_mood(random.choice(['excited', 'cheer']))
            self.say(random.choice(LUNA_USER_CAPTURE))
        else:
            self.luna_panel.set_mood('smile')
    ...
    def _apply_ai_move(self, ai_move):
        ...
        if self.board.is_check():
            self.luna_panel.set_mood('wink')
            self.say(random.choice(LUNA_CHECK))
        elif ai_capture:
            self.luna_panel.set_mood(random.choice(['smile', 'wink']))
    ...
    def announce_game_over(self):
        ...
        if outcome.winner is None:
            self.luna_panel.set_mood('sleepy')
            self.say(random.choice(LUNA_GAME_OVER_DRAW))
        elif outcome.winner == chess.WHITE:
            self.luna_panel.set_mood(random.choice(['sad', 'shy']))
            self.say(random.choice(LUNA_GAME_OVER_WIN))
        else:
            self.luna_panel.set_mood(random.choice(['victory', 'laughing']))
...
class LunaPanel(BoxLayout):
    ...
    AVATAR_SOURCES = {
        ...
        'angry': 'assets/luna_angry.png',
        'excited': 'assets/luna_excited.png',
        'cheer': 'assets/luna_cheer.png',
        'laughing': 'assets/luna_laughing.png',
        'victory': 'assets/luna_victory.png',
        'shy': 'assets/luna_shy.png',
        'sad': 'assets/luna_sad.png',
        'sleepy': 'assets/luna_sleepy.png',
        'smile': 'assets/luna_smile.png',
        'wink': 'assets/luna_wink.png',
        'love': 'assets/luna_love.png',
    }
    ...
    def set_mood(self, mood, duration=None):
        """Switch avatar to a mood. Moods now persist until something else
        explicitly changes them (rather than auto-reverting on a timer),
        so a quick sequence of events doesn't make the avatar flicker
        through several expressions in under a second."""
