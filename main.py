...
# Small per-square bonuses that push knights/bishops toward the center and
# discourage leaving the king exposed — this alone makes a shallow search
# look much less "aimless" without costing extra depth.
...
def evaluate_board(board):
    ...
    # Reward controlling/occupying the center
    ...
    # Penalize hanging pieces heavily — a piece that is attacked and not
    # defended is usually about to be lost, and this is the single biggest
    # source of "the AI just gave away a piece" behavior in shallow search.
    ...
    # Small bonus for king safety: castled/behind pawns is better than an
    # exposed king in the center once pieces have started trading off.
    total_material = sum(
        len(board.pieces(pt, chess.WHITE)) + len(board.pieces(pt, chess.BLACK))
        for pt in [chess.KNIGHT, chess.BISHOP, chess.ROOK, chess.QUEEN]
    )
    if total_material <= 6:  # endgame-ish: king activity matters more
        pass  # kept simple on purpose — avoids extra cost in the common case

    # Small bonus for having more legal moves (activity)
...
def ai_choose_move(board, difficulty='medium'):
    """difficulty controls search depth: 'easy' = mostly random with some
    tactical awareness, 'medium' = 2-ply lookahead, 'hard' = 3-ply lookahead."""
    legal_moves = list(board.legal_moves)

    if difficulty == 'easy':
        # Shallow 1-ply search most of the time, but occasionally just
        # play randomly so it stays beatable.
    ...
    def analyze_game(self):
        ...
        for i, move in enumerate(moves_played):
            if replay.turn == chess.WHITE:
                ...
                _, best_move = minimax(replay, 2, float('-inf'), float('inf'), True)
