...
def evaluate_board(board):
    ...
    # Small bonus for mobility (more legal moves = more active position)
    score += 0.05 * len(list(board.legal_moves)) * (1 if board.turn == chess.WHITE else -1)
...
def ai_choose_move(board, difficulty='medium'):
    ...
    if difficulty == 'hard':
        depth = 2  # Kept at 2-ply for now to stay responsive on phones;
                   # can be raised later once performance is confirmed.
