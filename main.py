...
def ai_choose_move(board, difficulty='medium'):
    ...
    if engine is not None:
        try:
            skill, think_time = {
                'easy': (3, 0.3), 'medium': (10, 0.6), 'hard': (18, 1.0),
            }.get(difficulty, (10, 0.6))
...
# ============================================
# Stockfish integration (attempt #2)
# ============================================
# Android blocks executing files copied straight out of the APK's
# 'assets' (read-only, noexec). This time we copy the bundled binary
# into the app's own private data directory first — a location the app
# owns and that isn't mounted noexec — then chmod +x and launch it from
# there. If any step fails, we silently fall back to the pure-Python
# engine above, so the app never crashes because of this.

STOCKFISH_ASSET_PATH = 'assets/stockfish'
_stockfish_engine = None
_stockfish_failed = False
STOCKFISH_STATUS = "not checked yet"


def _get_writable_stockfish_path():
    # Prefer Android's own storage API when running on-device — this is
    # the path Android itself considers the app's private, writable,
    # executable-capable directory.
    try:
        from kivy.utils import platform
        if platform == 'android':
            from android.storage import app_storage_path
            return os.path.join(app_storage_path(), 'stockfish_bin')
    except Exception:
        pass
    # Fallback (desktop testing, or if the android module isn't available)
    try:
        from kivy.app import App
        app = App.get_running_app()
        if app is not None:
            return os.path.join(app.user_data_dir, 'stockfish_bin')
    except Exception:
        pass
    return None
...
def get_stockfish_engine():
    global _stockfish_engine, _stockfish_failed, STOCKFISH_STATUS
    ...
    try:
        if not os.path.exists(STOCKFISH_ASSET_PATH):
            STOCKFISH_STATUS = f"bundled binary not found at {STOCKFISH_ASSET_PATH}"
            _stockfish_failed = True
            return None

        dest_path = _get_writable_stockfish_path()
        if dest_path is None:
            STOCKFISH_STATUS = "could not resolve a writable app data directory"
            _stockfish_failed = True
            return None

        # Copy into the writable, executable-capable directory (skip if
        # already copied from a previous run).
        if not os.path.exists(dest_path):
            shutil.copyfile(STOCKFISH_ASSET_PATH, dest_path)

        st = os.stat(dest_path)
        os.chmod(dest_path, st.st_mode | stat.S_IEXEC | stat.S_IXGRP | stat.S_IXOTH)

        _stockfish_engine = chess.engine.SimpleEngine.popen_uci(dest_path)
        STOCKFISH_STATUS = "OK - engine running"
        return _stockfish_engine
    except Exception as e:
        STOCKFISH_STATUS = f"failed at {dest_path if 'dest_path' in dir() else '?'}: {type(e).__name__}: {e}"
        _stockfish_failed = True
        return None
...
class ChessBoard(GridLayout):
    ...
    def on_square_press(self, instance):
        import time
        now = time.time()
        last_tap = getattr(self, '_last_tap_time', 0)
        if now - last_tap < 0.3:
            return  # ignore a second tap arriving within 300ms (touch double-fire)
...
class StartScreen(BoxLayout):
    ...
    def __init__(self, on_start, **kwargs):
        ...
        engine_label = Label(
            text=f"Engine: {STOCKFISH_STATUS}",
            font_size=11,
