...
def get_stockfish_engine():
    ...
    try:
        ...
        if not os.path.exists(STOCKFISH_PATH):
            STOCKFISH_STATUS = f"file not found at {STOCKFISH_PATH}"
        ...
        STOCKFISH_STATUS = f"failed to launch: {type(e).__name__}: {e}"
    _stockfish_failed = True
    return None
...
class StartScreen(BoxLayout):
    ...
    def __init__(self, on_start, **kwargs):
        ...
        # Diagnostic line: trigger a Stockfish launch attempt now and show
        # what happened, so engine problems are visible instead of silent.
