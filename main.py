...
def evaluate_board(board):
    ...
    # Reward controlling/occupying the center — this alone fixes a lot of
    # "aimless" looking moves from a shallow search.
    ...
    # Small bonus for having more legal moves (activity), computed cheaply
    # by counting rather than materializing the full move list.
...
class ChessBoard(GridLayout):
    ...
    def say(self, text):
        ...
        if TTS_AVAILABLE and self.sound_enabled:
    ...
    def analyze_game(self):
        ...
        engine considers the best alternative at that point."""
...
class ChessRoot(BoxLayout):
    ...
    def _build_layout(self):
        ...
        self.sound_btn = Button(
            text="Sound: On" if self.board_widget.sound_enabled else "Sound: Off",
            background_color=(0.29, 0.36, 0.46, 1),
            color=(1, 1, 1, 1),
        )
    ...
    def show_analysis(self, instance):
        ...
        scroll_text = ""
        if not flagged:
            scroll_text = "Nice game! I didn't spot any moves that clearly lost material or position."
        else:
            lines = [f"Found {len(flagged)} move(s) worth a second look:\n"]
            for f in flagged:
                lines.append(
                    f"Move {f['move_number']}: you played {f['played']} — "
                    f"{f['better']} was stronger."
                )
            scroll_text = "\n\n".join(lines)
