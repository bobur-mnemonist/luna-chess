...
class ChessBoard(GridLayout):
    ...
    def refresh_board(self):
        ...
        for idx, btn in self.squares.items():
            ...
            # Color the piece itself (white piece = bright fill, black
            # piece = dark fill) regardless of the square color beneath
            # it, so pieces stay legible on both light and dark squares.
...
class LunaPanel(BoxLayout):
    ...
    def _start_breathing(self):
        ...
        breathing = (
            Animation(opacity=0.85, duration=1.8, t='in_out_sine')
            + Animation(opacity=1.0, duration=1.8, t='in_out_sine')
        )
...
class ChessRoot(BoxLayout):
    def __init__(self, difficulty='medium', **kwargs):
        ...
        self.board_widget = ChessBoard(
            self.luna_panel, difficulty=self.difficulty, size_hint=(1, 0.68)
        )
...
class LunaChessApp(App):
    ...
    def start_game(self, difficulty):
        self.root_layout.clear_widgets()
        self.root_layout.add_widget(ChessRoot(difficulty=difficulty))
